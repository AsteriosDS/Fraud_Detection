...
def main():
    ...
    for i, row in enumerate(test_data.iterrows()):
        ...
        # Create clickable elements for each sample time series
        component_id = f"time_series_{i}"
        ...
        if selected_timeseries:
            ...
            # Plot original and reconstructed time series together
            ax.plot(selected_data, label="Original")
            ax.plot(reconstructed, label="Reconstructed")
            ax.fill_between(range(len(selected_data)), selected_data, reconstructed, where=(reconstructed >= selected_data), interpolate=True, color='green', alpha=0.5)
            ax.fill_between(range(len(selected_data)), selected_data, reconstructed, where=(reconstructed < selected_data), interpolate=True, color='red', alpha=0.5)
